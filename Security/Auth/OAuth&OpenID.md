...
The gateway is the thing that handles all of the requests from the Internet. That means it's out there handling requests with valid tokens, but also expired tokens. Also attackers trying to hack into the API, also just random junk being thrown at it. And also all these tokens that were previously valid but now have been revoked for some reason, like the user was deactivated. The API gateway is the first line of defense and it's handling every API request that comes through it. So it will be able to pass through or reject each request as quick as possible. So this is going to only do local validation. That lets it get all the speed benefits of local validation and avoid adding network traffic in order to validate tokens. It's able to quickly reject the obviously invalid tokens.  So that's the stuff from attackers, malformed tokens and also expired tokens. However, this local validation step doesn't have any way to be aware of a user being revoked. So those revoked tokens still look valid because they haven't expired yet. So it'll pass through both requests with valid tokens as well as requests with these revoked tokens. Now, if your access token lifetime is at most one hour, then your backend APIs only ever see requests from tokens that at the very least were valid within the last hour. So this is already a good start because what you've done in this point is move all your token validation logic into the gateway and you keep that logic out of the APIs themselves. Next, it's up to each API, or even individual methods within each API, to decide whether the fast validation that the gateway did was good enough. And in some cases, it might be. For example, for not super sensitive API operations, you might consider that local validation is good enough because the risk of leaking data via revoked tokens is pretty slim. So, for example, an API method that's going to return a customer's current rewards points balance. It's not a high risk method, especially if we already know the access token was valid within the last hour. But an API method that's about to go do something like charg a user's credit card or transfer money or ship an order, that definitely wouldn't want to run with a revoked token. So for the particular API methods that need the additional security, those ones on an individual basis can go back over the network and ask the token introspection endpoint whether the access token is actually still valid. since the authorization server is keeping track of the full state of the system, it'll be able to actually say, no, this token is not valid because it's been revoked. The nice thing about taking this hybrid approach is that you take advantage of the best parts of both ways of validating tokens.
